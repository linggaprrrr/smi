...
{                         
    ...
    public function exportDataStokProduct($date1= null, $date2 = null) {
        ...
        $sheet->setCellValue('F1', 'Penjualan Import Admin');
        $sheet->setCellValue('G1', 'Pengiriman');
        $sheet->setCellValue('H1', 'Sisa Stok Penjualan');
        $sheet->setCellValue('I1', 'Sisa Stok Gudang');
        $sheet->setCellValue('J1', 'HPP Gesit');
        $sheet->setCellValue('K1', 'HPP Jual');
        $sheet->setCellValue('L1', 'Nilai Barang');
        $sheet->setCellValue('M1', 'Nilai Jual');
        $sheet->setCellValue('N1', 'Pendapatan HPP');
        $i = 2;
        $no = 1;
        ...
        foreach($stok as $row) {
            ...
            $sheet->setCellValue('I' . $i, $row['sisa_gudang']);
            $sheet->setCellValue('J' . $i, $row['hpp']);
            $sheet->setCellValue('K' . $i, $row['hpp_jual']);
            $sheet->setCellValue('L' . $i, $row['hpp'] * $row['sisa_gudang']);
            $sheet->setCellValue('M' . $i, $row['hpp_jual'] * $row['sisa_gudang']);
            $sheet->setCellValue('N' . $i, ($row['hpp_jual'] * $row['sisa_gudang']) - ($row['hpp'] * $row['sisa']));
            $i++;
        }
    ...
    public function gudangLovishStokProduk() {
        ...
        if (is_null($date)) {
            ...
            $colors = $this->materialModel->getAllColors();
            $vendors = $this->productModel->getAllVendorPenjualan();
            $products = $this->productModel->getAllStockProductLovish($date1, $date2);
    ...
    public function stockOpname() {
        ...
        $stok = array();        
        $sisaStok = $this->productModel
                ->select('models.hpp, products.id, products.hpp_jual, products.model_id, products.color_id, products.size, scan_in, models.jenis as product_name, model_name, color, products.size, product_barcodes.updated_at, stok_masuk, penjualan, stok_retur, pengiriman, stok')
                ->join('models', 'models.id = products.model_id')
                ->join('colors', 'colors.id = products.color_id')
                ->join('product_barcodes', 'product_barcodes.product_id = products.id')
                ->join('(SELECT history_stok.model_id, history_stok.color_id, updated_at, SUM(history_stok.qty) as stok FROM history_stok WHERE jenis="sisa_pengiriman" AND status = 0 GROUP BY model_id, color_id, size) as a','products.model_id = a.model_id AND a.color_id = products.color_id', 'left') 
    ...
    public function exportSO() {
        ...
        foreach($stok as $product) {
            ...
            $sheet->setCellValue('C' . $i, $product['sisa_gudang']);            
    ...
    public function exportRetur($date1 = null, $date2 = null) {
        if (is_null($date1)) {
    ...
    public function exportProdukMasuk($date1 = null, $date2 = null) {
        if (is_null($date1)) {
            ...
            ->where("(product_barcodes.status = '2' OR product_barcodes.status = '3' OR product_barcodes.status = '5')")
            ->groupBy('product_barcodes.product_id')   
            ->get();
        }
        $date = date('m-d-Y H.i.s');
        $fileName = "Data Produk Masuk  {$date}.xlsx";  
        $spreadsheet = new Spreadsheet();
		$sheet = $spreadsheet->getActiveSheet();
		$sheet->setCellValue('A1', 'No');
		$sheet->setCellValue('B1', 'Produk');
    ...
    public function exportProdukKeluar($date1 = null, $date2 = null) {
        if (is_null($date1)) {
            ...
            $productsExp = $this->productModel
                ...
                ->join('models', 'models.id = products.model_id')
                ->join('colors', 'colors.id = products.color_id')
                ->join('(SELECT product_barcodes.product_id, product_barcodes.updated_at FROM product_logs JOIN product_barcodes ON product_barcodes.id = product_logs.product_id JOIN products ON products.id = product_barcodes.product_id WHERE product_logs.status = 3 OR product_logs.status = 5 AND product_barcodes.updated_at BETWEEN ".$date1." AND ".$date2.") as k', 'k.product_id = products.id')
                ->get();
        }
        $date = date('m-d-Y H.i.s');
    ...
    public function exportStokGudang($date1 = null, $date2 = null) {
        if (is_null($date1)) {
            $sisaStok = $this->productModel
                ->select('models.hpp, products.id, products.hpp_jual, products.model_id, products.color_id, products.size, models.jenis as product_name, model_name, color, products.size, product_barcodes.updated_at, stok_masuk, penjualan, stok_retur, pengiriman, stok')
                ->join('models', 'models.id = products.model_id')
